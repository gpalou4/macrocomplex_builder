...
def MacrocomplexBuilder(ref_structure, files_list, RMSD, clashes, number_chains, it):
	...
	file_path = arguments.indir + "/" + sample 			#takes the path of the sample file
	...
	if superimposed_chains is False or best_RMSD > RMSD_threshold:		#if condition is met, there are no superimposed chains, or the RMSD is not small enough to be considered
		...
		return MacrocomplexBuilder(ref_structure = ref_structure, files_list = files, RMSD = RMSD_threshold, clashes = clashes_threshold, number_chains = nc, it = i)	#call again the iterative function, j does not change
		...
		for chains, sup in all_superimpositions:
			...
			if present_chain is False:						
				...
				if arguments.iterations:		
					os.chdir(os.path.abspath(arguments.outdir))		#changes the current directory to output directory
					...
					io.save("macrocomplex_iteration_%d.pdb" %(i))	#saves the structure on a file
					os.chdir("../")									#changes the current directory back to where the input directory is located
				break							#breaks loop
	...
	return MacrocomplexBuilder(ref_structure = ref_structure, files_list = files, RMSD = RMSD_threshold, clashes = clashes_threshold,  number_chains = nc, it = i)		
...
parser.add_argument('-i', '--input',			#INPUT FOLDER argument
					dest = "indir",
					action = "store",
					required=True,
					help = "Input folder (or path) containing all .pdb files with the protein interactions.")	

parser.add_argument('-o', '--output',			#OUTPUT FOLDER argument
					dest = "outdir",
					action = "store",
					default = None,
					help = "Output folder (or path) containing all the models generated in each iteration and the final macrocomplex structure in .pdb format.")

parser.add_argument('-v', '--verbose',			#VERBOSE argument
					dest = "verbose",
					action = "store_true",
					default = False,
					help = "Progression log printed in standard output file.")

parser.add_argument('-it', '--iterations',		#ITERATIONS argument
					dest = "iterations",
					action = "store_true",
					default = False,
					help = "If set, each iteration is saved in a different.pdb file.")
...
parser.add_argument('-nc', '--number_chains',		#NUMBER OF CHAINS argument
					...
					required=True,
					type = int,
					help = "Number of chains desired for the target complex.")
...
parser.add_argument('-rmsd', '--rmsd_threshold',		#RMSD THRESHOLD argument
					...
					default = 0.3,
					type = float,
					help = "If set, the RMSD threshold for considering a superimposition as correct will take this value, if not, it will be 0.3 by default. The output of the program is very sensitive to this value, careful when modifying it.")

parser.add_argument('-cl', '--clashes',		#CLASHES argument
					dest = "clashes",
					action = "store",
					default = 30,
					type = int,
					help = "If set, the threshold of the number of clashes will take this value, if not, it will be 30 by default. The output of the program is very sensitive to this value, careful when modifying it.")
...
MacrocomplexBuilder(ref_structure = ref_structure, files_list = files, RMSD = arguments.rmsd_threshold, clashes = arguments.clashes, number_chains = arguments.number_chains,  it = 0)	#calling the iterative function
